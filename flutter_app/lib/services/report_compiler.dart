...
class LocalReportCompiler {
  static String generateWshReport({
    ...
    // Extract individual hazard evaluation blocks safely from your existing JSON schema
    ...
    String getAiField(Map<String, dynamic> block, String field) {
      return block[field]?.toString().trim() ?? 'N/A';
    }
    ...
    // Compile all data streams into the final 9-section report template
    return '''
# WORKPLACE SAFETY & HEALTH (WSH) OFFICIAL INSPECTION REPORT

## 1. Site and Personnel Details
* **Inspection Date:** $currentDate
* **Inspection Time:** $currentTime SGT
* **Site Location / Designated Zone:** ${locationCtrl.text.isEmpty ? "Not Specified" : locationCtrl.text}
...
### Supplemental Technician On-Site Observations
${manualNotesCtrl.text.isEmpty ? "No manual auxiliary hazards logged by the inspector." : manualNotesCtrl.text}
...
1. **Scaffolding / Elevation Tasks Risk Level:** ${getAiField(ladder, 'compliance')}
   * *Analytical Reasoning:* ${getAiField(ladder, 'reasoning')}
2. **Personnel Protective Gear Compliance Level:** ${getAiField(ppe, 'compliance')}
   * *Analytical Reasoning:* ${getAiField(ppe, 'reasoning')}
3. **Buddy System / Operational Supervision Adherence:** ${getAiField(buddy, 'compliance')}
   * *Analytical Reasoning:* ${getAiField(buddy, 'reasoning')}

---

## 4. Preventive and Corrective Measures
### Immediate Mandatory Mitigations (On-Site Directives)
...
* **Analysis Context:** Visual evaluation cross-referenced automatically against standard industrial compliance parameters. The logged descriptive breakdowns in Section 2 serve as the primary legal audit trail for this visual evidence.

---

## 6. Review of Changes
### Operational Shift & Process Transitions Cumulative Safety Impact
${changesCtrl.text.isEmpty ? "No recent process, machinery configurations, or major personnel shifts declared for this inspection cycle." : changesCtrl.text}

---

## 7. Worker / Subcontractor Feedback
### Documented On-Site Personnel Concerns & Engineering Action Path
${feedbackCtrl.text.isEmpty ? "No complaints, safety concerns, or procedural feedback raised by subcontractors/workers during this site walk." : feedbackCtrl.text}

---

## 8. Construction / Site Supervision
* **Structural Plan Adherence & Joint Safety Evaluation:** ${getAiField(hazards, 'reasoning').contains('structural') ? getAiField(hazards, 'reasoning') : "Structural layout and steel connection joints match standard operational guidelines within the visible camera range."}
