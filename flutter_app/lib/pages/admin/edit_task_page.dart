...
class EditTaskPage extends StatefulWidget {
  ...
  task; // Optional: Pass existing task data for editing
...
class _EditTaskPageState extends State<EditTaskPage> {
  ...
  Future<void> _loadCurrentAssignments() async {
    try {
      ...
      setState(() {
        selectedTechnicianIds = techData
            .map((t) => t['technician_id'].toString())
            .toList();
        selectedSWPIds = swpData
            .map((s) => s['swp_template_id'].toString())
            .toList();
      });
    } catch (e) {
  ...
  Future<void> assignTask() async {
    ...
    try {
      ...
      final techAssignment = selectedTechnicianIds
          .map(
            (techId) => {'task_id': taskId, 'technician_id': int.parse(techId)},
          )
          .toList();
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('Task edited successfully')));

      await Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) =>
              AllTasksPage(), // Refresh page with updated data
        ),
      );
    } catch (e) {
      ScaffoldMessenger.of(
