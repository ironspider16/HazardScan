...
//     const cleanBase64 = imageBase64.replace(/^data:image\/[a-z]+;base64,/, ""); // Image always provided
  ...
  const buildDiagnosticJson = (title: string, details: string) => {
    return JSON.stringify({
      ...
      ladderHeight: {
        ...
        reasoning: details,
        advice:
          "Check this text payload to see exactly what Google returned to your edge function.",
      },
      ppe: {
        compliance: "N/A",
        description: "N/A",
  ...
  try {
    ...
    if (!hasImage && !hasText) {
      return new Response(
        buildDiagnosticJson(
          "Missing Input",
          "No image or text was provided. Please upload an image or enter technician context."
        ),
        {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
          status: 200,
...
ADDITIONAL SITE CONTEXT PROVIDED BY THE TECHNICIAN ON-SITE:
"${hasText ? userContext : "No additional context provided."}"

HOSPITAL SWP CONSTRAINTS TO ENFORCE:
1. LADDERS & HEIGHT:
Check for locked spreader bars, standing on the top rung, carrying items, lack of 3-point contact, and unstable placement.
If no image is provided and the text does not mention ladder hazards, mark as "N/A".

2. BUDDY SYSTEM & PERSONNEL:
Check for lack of buddy system on ladders, missing attendants/watchmen, and unauthorized entry.
If no image is provided, only judge based on the technician text.

3. PPE:
Look for missing safety helmets, safety shoes, gloves, and rubber boots on wet surfaces.
If no image is provided, only judge based on the technician text.

4. AREA & SURFACE HAZARDS:
Check for uncordoned work areas, slippery/wet surfaces, blocked routes/exits, debris, confined spaces, poor ventilation, and general hazards.

OUTPUT INSTRUCTIONS:
- Return only valid JSON.
- Evaluate the 4 target safety categories.
- Use the fields: compliance, description, reasoning, advice.
- For compliance, choose exactly one:
[DANGEROUS, PARTIALLY COMPLIANT, COMPLIANT, SAFE, N/A]
- Do not invent details.
- If no image is provided, clearly state that the judgement is based only on text context.
`;
    ...
    for (const key of apiKeys) {
      try {
        ...
        if (hasImage) {
          ...
          result = await model.generateContent([
            prompt,
            `Technician text report: ${userContext}`,
          ]);
        }
        ...
        if (
          ...
          return new Response(
            buildDiagnosticJson(
              "Empty Candidates Object",
              `API responded but candidates array is empty. Full response: ${JSON.stringify(
                result
              )}`
            ),
            {
        ...
        if (!textResponse || textResponse.trim() === "") {
          return new Response(
            buildDiagnosticJson(
              "Empty Text Response",
              "result.response.text() returned empty."
            ),
            {
              headers: { ...corsHeaders, "Content-Type": "application/json" },
              status: 200,
        ...
        return new Response(
          buildDiagnosticJson(
            "SDK Execution Error",
            err.message || "Unknown internal SDK error."
          ),
          {
            headers: { ...corsHeaders, "Content-Type": "application/json" },
            status: 200,
    ...
    return new Response(
      buildDiagnosticJson(
        "Exhaustion Error",
        "All configured API keys returned quota failures."
      ),
      {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
        status: 200,
