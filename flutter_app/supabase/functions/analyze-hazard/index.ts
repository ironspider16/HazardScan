...
serve(async (req: Request) => {
  ...
  try {
    // 1. Accept an array of images from the mobile frontend
    ...
    // 2. Sanitize and prepare all images as dynamic inline parts for Gemini
    ...
    // 3. Prompt wording updated to command a collective analysis of multiple viewpoints
...
`;
    ...
    for (const key of apiKeys) {
      try {
          ...
          generationConfig: {
            ...
            responseSchema: {
              ...
              properties: {
                overallStatus: {
                  type: "STRING",
                  description: "The highest severity status across all categories.",
                  enum: ["DANGEROUS", "PARTIALLY COMPLIANT", "COMPLIANT", "SAFE", "N/A"]
                },
                ladderHeight: {
                  type: "OBJECT",
                  properties: { compliance: { type: "STRING", enum: ["DANGEROUS", "PARTIALLY COMPLIANT", "COMPLIANT", "SAFE", "N/A"] }, description: { type: "STRING" }, reasoning: { type: "STRING" }, advice: { type: "STRING" } },
        ...
        // 4. Spread out all image parts into the execution request array natively
        ...
        if (!textResponse || textResponse.trim() === "") {
          return new Response(buildDiagnosticJson("Empty Text Response", "result.response.text() returned empty. Candidates existed but text extraction failed."), {
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
            status: 200
          });
        }
        ...
        console.log(`Processed ${imageParts.length} image inputs.`);
